...
def test_parse_and_validate_ldif_ok(monkeypatch: pytest.MonkeyPatch, service: FlextDbtLdifService, tmp_path: Path) -> None:
    monkeypatch.setattr(
        service.client,
        "parse_ldif_file",
...
def test_generate_and_write_models_ok(monkeypatch: pytest.MonkeyPatch, service: FlextDbtLdifService) -> None:
    ...
    def _write(models: list[Any], *, overwrite: bool = False) -> FlextResult[dict[str, object]]:
...
def test_simple_api_helpers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    def _run(self: FlextDbtLdifService, ldif_file: Path | str, *, generate_models: bool = True, run_transformations: bool = False, model_names: list[str] | None = None) -> FlextResult[dict[str, object]]:
    ...
    def _run_quality(self: FlextDbtLdifService, ldif_file: Path | str) -> FlextResult[dict[str, object]]:
        return FlextResult.ok({"ok": True})

    monkeypatch.setattr(FlextDbtLdifService, "run_data_quality_assessment", _run_quality)
    assert validate_ldif_quality(tmp_path / "f.ldif").success

    def _parse_val(self: FlextDbtLdifService, ldif_file: Path | str) -> FlextResult[dict[str, object]]:
        return FlextResult.ok({"entries": []})

    def _gen_models(self: FlextDbtLdifService, entries: list[object], *, overwrite: bool = False) -> FlextResult[dict[str, object]]:
